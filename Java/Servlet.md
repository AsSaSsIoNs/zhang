...
访问一个Servlet后跳转至另一个Servlet

*   ``./src/``下新建两个Servlet`RequestDemo`和`RequestDemo2`
...
*   ```java
    ...
    public class RequestDemo extends HttpServlet {
        @Override
        protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
            System.out.println("执行了RequestDemo");
            req.getRequestDispatcher("/RequestDemo2").forward(req, resp);
            //使用getRequestDispatcher来转发，后面接另一个Servlet的路径
        }
        @Override
        protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
            doPost(req, resp);
...
public class RequestDemo2 extends HttpServlet {
    	@Override
    	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        System.out.println("执行了RequestDemo2");
    	}
@Override
    	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        doPost(req, resp);
...
*   浏览器访问` http://localhost:8080/RequestDemo `时，控制台输出如下![1571361571714](1571361571714.png)
*   但是只能在服务器内部转发，如果将`getRequestDispatcher`参数设置为一个网页就会404
*   看似访问了两个Servlet，但是只有一次请求，在Chrome控制台中的结果是这样![1571361712644](1571361712644.png)
*   在转发后浏览器地址栏的请求路径也没有变化，不像`doGet`那样会显示参数

### 数据共享

一个Servlet能存取另一个Servlet的数据

*   修改`RequestDemo`和`RequestDemo2`的内容

    ```java
    //RequestDemo
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
            System.out.println("执行了RequestDemo");
            req.setAttribute("name", "zhang");
            req.getRequestDispatcher("/RequestDemo2").forward(req, resp);
        }
    //RequestDemo1
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
...
*   在`RequestDemo`中设置参数，使用`RequestDemo1`来测试能否访问到这个参数

*   控制台输出为![1571362412845](1571362412845.png)
