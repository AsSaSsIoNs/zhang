...
*    ```java
    ...
    注解的urlPatterns参数表示了此Servlet的名称，req.getParameter用来将参数实际值提取出来建立user对象，再新建Dao(Data Access Object)与数据库交互，传入要交互的对象user，输出交互结果，如果存在这个记录就跳转至成功页面，如果失败就跳转至失败页面
...
*    ```java
    ...
    与数据库的交互程序，第2行先使用自定的工具类来连接数据库，login方法先声明了sql语句，使用template自带的查询fang
