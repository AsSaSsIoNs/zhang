...
>增强实验。 当使用带有小对象的原始图像及其带有复制粘贴小对象的副本进行训练时，无论是在小对象上还是在总体上，都可以实现最佳性能。
...
>复制粘贴多个对象。 与单个对象的复制粘贴相比，制作更多副本更好

* Copy-pasting of all small objects
>Table 7，列出复制粘贴每个图像中所有小目标的结果
>在对所有小目标只进行一次 augmention 时，得到了最佳的分割和检测结果
>我们猜测这背后可能有两个原因
...
为了解决这一问题，我们提出了对原有的 MS COCO 数据集进行 augmentation 操作：首先通过对包含小目标的图像进行 oversampling，可以很容易地提高小对象的性能。其次，提出了一种基于复制粘贴小对象的 augmentation 算法。与目前最先进的方法 Mask R-CNN 相比，我们在 MS COCO 上实现了 9.7%9.7% 的实例分割和 7.1%7.1% 的目标检测相对改进。实验证明，所提出的增强方法可以在对小目标和大目标的预测质量之间进行权衡。 
